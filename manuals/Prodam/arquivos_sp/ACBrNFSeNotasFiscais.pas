...
begin
 ...
 try
  ...
  {add-sp}
  //Adicionado para a Assinatura do RPS diferenciado no Provedor SP
  LocNFSeW.NumSerCert     :=TACBrNFSe( TNotasFiscais( Collection ).ACBrNFSe ).Configuracoes.Certificados.NumeroSerie;
 ...
 for i := 0 to Self.Count-1 do
  begin
   ...
   try
    ...
    //Adicionado para a Assinatura do RPS diferenciado no Provedor SP
    {add-sp}
    LocNFSeW.NumSerCert     := self.Configuracoes.Certificados.NumeroSerie;
    LocNFSeW.GerarXml;
...
begin
 try
               ...
               then Tipo := 3
                    ...
                    then Tipo := 4
                         ...
                         then Tipo := 5
                              ...
                              then Tipo := 6
                              else if Pos('</NFe>', ArquivoXML.Text) > 0 then Tipo:=7
                                   else Tipo := 0;
  ...
  case Tipo of
   ...
   7: begin //SP
        CNPJ:= Copy(ArquivoXML.Text,
                    Pos('<CPFCNPJPrestador>', ArquivoXML.Text) + 18,
                    Pos('</CPFCNPJPrestador>',ArquivoXML.Text) - (Pos('<CPFCNPJPrestador>', ArquivoXML.Text) + 18));
        CNPJ:=StringReplace(CNPJ, '<CNPJ>', '', [rfReplaceAll]);
        CNPJ:=StringReplace(CNPJ, '</CNPJ>', '', [rfReplaceAll]);
        CNPJ:=StringReplace(CNPJ, '<CPF>', '', [rfReplaceAll]);
        CNPJ:=StringReplace(CNPJ, '</CPF>', '', [rfReplaceAll]);
        IM:= Copy(ArquivoXML.Text,
                  Pos('<InscricaoPrestador>', ArquivoXML.Text) + 20,
                  Pos('</InscricaoPrestador>',ArquivoXML.Text) - (Pos('<InscricaoPrestador>', ArquivoXML.Text) + 20));

        while pos('</NFe>',ArquivoXML.Text) > 0 do
          begin
            XML            := Copy(ArquivoXML.Text, Pos('<NFe>', ArquivoXML.Text), Pos('</NFe>', ArquivoXML.Text) + 5);
            ArquivoXML.Text:= Trim(Copy(ArquivoXML.Text, Pos('</NFe>', ArquivoXML.Text) + 6, Length(ArquivoXML.Text)));
            LocNFSeR       := TNFSeR.Create(Self.Add.NFSe);
            //LocNFSeR.Provedor:= proSP;
            LocNFSeR.NFSe.Prestador.Cnpj:= CNPJ;
            LocNFSeR.NFSe.Prestador.InscricaoMunicipal:= IM;
            try
              LocNFSeR.Leitor.Arquivo := XML;
