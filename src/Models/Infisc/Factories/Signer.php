 ...
 * Meets only for Infisc
 *
 * @category  NFePHP
 * @package   NFePHP\NFSe\Models\Infisc\Factories\Signer
...
class Signer
{
    ...
    public static function sign(
        ...
        if (empty($node) || empty($root)) {
            throw \NFePHP\Common\Exception\SignerException::tagNotFound($tagname . ' ' . $rootname);
        }
    ...
    private static function createSignature(
        ...
        $x509SubjectNameNode = $dom->createElement('X509SubjectName', self::buildX509SubjecName($certificate));
    ...
    private static function buildX509SubjecName(Certificate $certificate)
    {
        $x509_cert = openssl_x509_read($certificate);
        $x509_parse = openssl_x509_parse($x509_cert);
        $subject = [];
        $subject[] = 'CN='.$x509_parse['subject']['CN'];
        foreach ($x509_parse['subject']['OU'] as $key => $value){
            $subject[] = 'OU='.$value;
        }
//        $subject[] = 'OU=';
//        $subject[] = 'OU=';
...
//        $subject[] = 'OU=';
//        $subject[] = 'OU=';
//        $subject[] = 'OU=28504634000109';
//        $subject[] = 'OU=';
        $subject[] = 'O='.$x509_parse['subject']['O'];
        $subject[] = 'L='.$x509_parse['subject']['L'];
        $subject[] = 'ST='.$x509_parse['subject']['ST'];
        $subject[] = 'C='.$x509_parse['subject']['C'];
